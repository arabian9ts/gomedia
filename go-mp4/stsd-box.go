...
type Tx3gSampleEntry struct {
	entry                   *SampleEntry
	horizontal_justifcation int8
	vertical_justifcation   int8
	bg_color_rgba           [4]byte
	box_record              [4]int16
	style_record            [12]byte
}

func NewTx3gSampleEntry(format [4]byte) *Tx3gSampleEntry {
	return &Tx3gSampleEntry{
		entry: NewSampleEntry(format),
	}
}

func (entry *Tx3gSampleEntry) Size() uint64 {
...
func (entry *Tx3gSampleEntry) Encode() (int, []byte) {
	...
	buf[offset] = byte(entry.horizontal_justifcation)
	offset++
	buf[offset] = byte(entry.vertical_justifcation)
	offset++
	copy(buf[offset:offset+4], entry.bg_color_rgba[:])
	offset += 4
	for i := 0; i < 4; i++ {
		binary.BigEndian.PutUint16(buf[offset:], uint16(entry.box_record[i]))
		offset += 2
	}
...
func (stsd *SampleDescriptionBox) Encode() (int, []byte) {
	offset, buf := stsd.box.Encode()
	binary.BigEndian.PutUint32(buf[offset:], stsd.entry_count)
	offset += 4
	return offset, buf
}
...
func makeStsd(track *mp4track, handler_type HandlerType) []byte {
	...
	if handler_type.equal(vide) {
		...
		entry.entry.box.Size = entry.Size() + uint64(len(avbox))
		offset, se = entry.Encode()
	} else if handler_type.equal(sbtl) {
		entry := NewTx3gSampleEntry(getCodecNameWithCodecId(track.cid))
		entry.horizontal_justifcation = 1
		entry.vertical_justifcation = -1
		entry.bg_color_rgba = [4]byte{0, 0, 0, 255}
		entry.box_record = [4]int16{0, 0, 0, 0}
		entry.style_record = [12]byte{0, 0, 0, 0, 0, 1, 0, 16, 255, 255, 255, 255}
		entry.entry.box.Size = entry.Size() + uint64(len(avbox))
		offset, se = entry.Encode()
	}
